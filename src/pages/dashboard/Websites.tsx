  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-6xl flex-col p-4 text-slate-950 md:p-8">
      <div className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">Access and manage previously generated projects.</p>
      </div>

      {loading ? (
        <div className="flex flex-1 items-center justify-center text-slate-500">Loading websites...</div>
      ) : websites.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center rounded-[2rem] border border-dashed border-slate-200 bg-white p-8 text-slate-500">
          <LayoutPanelLeft className="mb-4 h-12 w-12 text-emerald-600/50" />
          <p>No generated websites found.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {websites.map((website) => (
            <div key={website.id} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-emerald-300">
              <div className="relative grid h-40 place-items-center bg-gradient-to-br from-emerald-500/20 via-slate-950 to-black">
                ...
                <Code className="relative h-10 w-10 text-emerald-700/70" />
              </div>
              <div className="p-5">
                <h3 className="truncate text-lg font-black text-slate-950">{website.title || "Untitled Project"}</h3>
                <p className="mt-1 text-sm text-slate-500">{website.websiteType || "AI Generated Website"}</p>
                <p className="mt-4 flex items-center gap-2 text-xs text-slate-500">
                  <Clock className="h-3 w-3" /> Generated on {new Date(website.createdAt || Date.now()).toLocaleDateString()}
                </p>
                <button onClick={() => handleDelete(website.id)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm font-bold text-red-700 hover:bg-red-100">
